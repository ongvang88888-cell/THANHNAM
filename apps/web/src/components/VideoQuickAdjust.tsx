...
export function VideoQuickAdjust(props: {
  ...
  return (
    <div className="quick-adjust">
      <h4>Chỉnh nhanh sau AI</h4>
      <p className="muted">Cắt đầu/cuối và chọn ảnh bìa. Không chạy lại toàn bộ công thức.</p>
      {previewUrl && (
        <video
          ref={videoRef}
          className="ai-edit-preview"
          src={previewUrl}
          controls
          preload="metadata"
        />
      ...
      <div className="quick-adjust-actions">
        <button type="button" disabled={props.disabled || busy !== null} onClick={() => void handleTrim()}>
          {busy === "trim" ? "Đang cắt…" : "Cắt đoạn đã chọn"}
        </button>
        <button
          type="button"
          className="secondary"
        ...
        >
          {busy === "thumb" ? "Đang lấy khung…" : "Đặt ảnh bìa tại đây"}
        </button>
      </div>
