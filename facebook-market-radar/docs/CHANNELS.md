...
UI: menu trái nhóm **Nền tảng / sàn** (Shopee đầu tiên, rồi Lazada / Google / YouTube…) + pill trên header + `/xu-huong` hiện 999 tên kênh đang chọn (mặc định Shopee) · chip trên `/`, `/xu-huong`, `/kenh/[…]`, `/top/[…]` · `/tong-hop` · API: `GET /api/kenh?tab=` + `GET /api/top?tab=` + `GET /api/tong-hop` (`estimated: true`, `autoCrawl: false`, `facebookNationalDump: false`, `nationalSalesDump: false`) · ghi thêm: `POST /api/kenh`.
