...
type Props = { searchParams: Promise<{ group?: string; niche?: string; asOf?: string }> };
...
export default async function ScanPage({ searchParams }: Props) {
  ...
  const plan = await getRadarService().scanPlan(nowMs);

  return (
    <>
      <h1>Hàng đợi quét nhiều cành</h1>
      <p className="muted">
        {plan.totalBranches} cành từ khóa (danh mục + nhánh sản phẩm) để bạn tự mở Thư viện quảng cáo
        Meta — quốc gia VN, trạng thái đang chạy. Radar chỉ biết ads <strong>đã lưu</strong>. Điểm nóng
        vẫn ước lượng.
      </p>
